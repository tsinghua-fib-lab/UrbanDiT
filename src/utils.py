...
def select_batch_size(args, dataset):
    ...
    if 'Gjh' in dataset:
        batch_size = int(args.batch_size_nj * args.batch_ratio)
    if 'Chy' in dataset or 'Dianxin' in dataset:
        batch_size = int(args.batch_size_Pop * args.batch_ratio)
    if 'TaxiBJ' in dataset or 'TrafficG' in dataset:
    ...
    if 'PEMS' in dataset:
        batch_size = int(args.batch_size_graph_small * args.batch_ratio)
    if 'METR_LA' in dataset or 'PEMS_BAY' in dataset:
        batch_size = int(args.batch_size_graph_small * 2  * args.batch_ratio)
    if  'los_speed' in dataset:
        batch_size = int(args.batch_size_graph_small//4  * args.batch_ratio)

    return batch_size

def select_patch_size(args, data):

    MIN, MID, MAX = [int(i) for i in args.multi_patch_size.split('-')]
    
    if 'Gjh' in data or 'TaxiBJ' in data or 'Dianxin' in data or 'TaxiNYC' in data or 'Chy' in data:
        patch_size = MIN
    ...
    if 'PEMS' in data or 'METR_LA' in data or 'los_speed' in data:
        patch_size = 1

    if 'PEMS_04' in data and 'Graph' not in data:
        patch_size = 1
